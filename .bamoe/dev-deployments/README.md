...
For this repository, the OpenShift option runs the complete Spring Boot project with the Maven
Wrapper supplied by the Canvas Dev Deployment base image. It uses
`config/settings-bamoe-openshift.xml` to resolve IBM BAMOE artifacts from the cluster-internal
Maven Repository Service. The Kubernetes option remains a generic accelerator reference and is
not part of this OpenShift PoC.
